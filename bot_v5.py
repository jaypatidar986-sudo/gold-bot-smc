...
# CONFIG
# ============================================================
BOT_TOKEN = "8941406579:AAFy7sk6aW6ltjPF7WwFf7k2cSHAcVhD3OA"
CHAT_ID = "5885172416"
...
if state.get('startup_date') != today:
    if send_tg(f"🤖 <b>USDJPY Bot V5 Started</b>\n\n✅ 1H signals (2-3/day)\n✅ 1M reversal check\n✅ 15min updates\n\n{datetime.now(IST).strftime('%d-%b %H:%M IST')}"):
...
while True:
    try:
        ...
        if is_weekend():
            print(f"[{now_utc.strftime('%H:%M')}] Weekend — sleeping")
            time.sleep(3600)
            continue

        # Fetch 1H
        if now - last_1h_fetch >= DATA_1H_CACHE_SEC or df1h is None:
            d = get_1h()
            if d is not None:
                df1h = add_ind(d)
                last_1h_fetch = now
                print(f"[{now_utc.strftime('%H:%M')}] 1H refreshed")

        # Fetch 1M
        if now - last_1m_fetch >= DATA_1M_CACHE_SEC or df1m is None:
            d = get_1m()
            if d is not None:
                df1m = d
                last_1m_fetch = now
                print(f"[{now_utc.strftime('%H:%M')}] 1M refreshed")

        # Signal + Reversal Check (60 sec)
        if now - last_signal_check >= SIGNAL_CHECK_SEC:
            ...
            if df1m is not None and state.get('active_signal'):
                ...
                if r:
                    ...
                    send_tg(msg_rev(r, a))
                    print(f"[{now_utc.strftime('%H:%M')}] {r['type']} @ {r['price']:.3f}")
                ...
                if cooldown_ok:
                    ...
                    if sigs:
                        ...
                        if send_tg(msg_signal(s)):
                            print(f"[{now_utc.strftime('%H:%M')}] ✅ {s['dir']} @ {s['entry']:.3f}")
        ...
        # Price update (15 min, only active trade)
        if now - last_price_send >= PRICE_UPDATE_MIN * 60:
            if df1m is not None and state.get('active_signal'):
                send_tg(msg_price(df1m, state))
                print(f"[{now_utc.strftime('%H:%M')}] 💰 Price update")
            last_price_send = now
